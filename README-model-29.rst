...
.. |logo| image:: hikaru-model-29-logo.png
   :alt: Hikaru
...
- Hikaru provides type-annotated classes that model all of the Kubernetes resources in Python
  and supports CRUD operations on those classes to manage their lifecycle in your Kubernetes cluster. These
  classes include the documentation from the source K8s swagger so they can serve as a single source of
  info for working with Kubernetes using Hikaru.
